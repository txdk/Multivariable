...
\[ O^{-1}=O^T \]
i.e the matrix \( O \) is an orthogonal matrix. This reveals that orthogonal transformations between Euclidean spaces are simply the ones that map orthonormal bases to orthonormal bases.
...
\end{definition}
  ...
  The dual space of a vector space is itself a vector space under the operations of addition and scalar multiplication of real-valued functions.
...
\begin{theorem}
  Let \( V \) be an \( n \)-dimensional vector space. Then \( V \) is isomorphic to its second dual \( V^{**} \).
\end{theorem}

\begin{proposition}
  Let \( V \) be an \( n \)-dimensional normed vector space. Then \( V^* \) is a normed space, with norm given by:
  \[ \norm{f}=\sup\qty{\norm{f(x)}\mid x\in V, \norm{x}= 1} \]
\end{proposition}

\begin{theorem}
  Let \( E \) be a Euclidean \( n \)-space. Then its dual space \( E^* \) is also a Euclidean \( n \)-space.
\end{theorem}
